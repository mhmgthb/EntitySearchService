...
@Slf4j
@RestControllerAdvice(name = "exceptionHandler")
//@SuppressWarnings()
public class SearchServiceExceptionHandlerImpl  {
    ...
    public Mono<ResponseEntity<Map<String, String>>> handleServiceException(ServiceException ex) {
        ...
        }else if(ex.getErrorCode() == ErrorCodes.NOT_FOUND){
            log.error(ex.getMessage(),ex);
            return generateResponse(ex.getMessage(), HttpStatus.NOT_FOUND);
        }else if(ex.getErrorCode() == ErrorCodes.NOT_IMPLMENTED){
            log.error(ex.getMessage(),ex);
            return generateResponse(ex.getMessage(), HttpStatus.NOT_IMPLEMENTED);
    ...
    public Mono<ResponseEntity<Map<String, String>>> handleProductNotFoundException(NoResourceFoundException ex) {
        log.info("---------------------NoResourceFoundException------------------------------");
        log.error("Handling " + ex.getClass().getSimpleName() + " due to " + ex.getMessage());

            return generateResponse(CommonConstants.BAD_REQUEST, HttpStatus.BAD_REQUEST);
    ...
    public Mono<ResponseEntity<Map<String, String>>> handleProductNotFoundException(MethodNotAllowedException ex) {
        log.info("---------------------NoResourceFoundException------------------------------");
        log.error("Handling " + ex.getClass().getSimpleName() + " due to " + ex.getMessage());

        return generateResponse(ex.getMessage(), HttpStatus.METHOD_NOT_ALLOWED);
   ...
   // @ResponseStatus(HttpStatus.BAD_REQUEST)
    ...
    public Mono<ResponseEntity<Map<String, String>>> handleValidationException(ServerWebInputException ex) {
    ...
    protected  Mono<ResponseEntity<Map<String, String>>> generateResponse(String message, HttpStatus status) {
        Map<String, String> map = new HashMap<String, String>();
        map.put("message", message);
        map.put("status", String.valueOf(status.value()));
        //map.put("data", responseObj);

        return Mono.just(new ResponseEntity<Map<String, String>>(map,status));
    }
